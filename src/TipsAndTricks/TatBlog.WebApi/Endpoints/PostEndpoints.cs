...
namespace TatBlog.WebApi.Endpoints
{
    public class PostEndpoints : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            ...
            routeGroupBuilder.MapGet("/filter", GetFilter)
              .WithName("GetFilterData")
              .Produces<ApiResponse<PostFilterData>>();
            ...
            IAuthorRepository authorRepository)
        {
            var authors = await authorRepository
                .GetAllAuthorsAsync<AuthorDto>(
                    authors => authors.ProjectToType<AuthorDto>());

            var categories = await blogRepository
                .GetCategoriesAsync<CategoryDto>(
            ...
            return Results.Ok(ApiResponse.Success(postFilterData));
        }
